...
I353_hot     = hp.read_map(version+'/data/dust_hot_band27_k00010.fits',verbose=False)
I545_hot     = hp.read_map(version+'/data/dust_hot_band31_k00010.fits',verbose=False)
I857_hot     = hp.read_map(version+'/data/dust_hot_band34_k00010.fits',verbose=False)
I3000_hot    = hp.read_map(version+'/data/dust_hot_band49_k00010.fits',verbose=False)

I353_cold    = hp.read_map(version+'/data/dust_cold_band27_k00010.fits',verbose=False)
I545_cold    = hp.read_map(version+'/data/dust_cold_band31_k00010.fits',verbose=False)
I857_cold    = hp.read_map(version+'/data/dust_cold_band34_k00010.fits',verbose=False)
I3000_cold   = hp.read_map(version+'/data/dust_cold_band49_k00010.fits',verbose=False)

dust_T_hot   = hp.read_map(version+'/data/dust_hot_Td_c0002_k00010.fits',verbose=False)
dust_b_hot   = hp.read_map(version+'/data/dust_hot_beta_c0002_k00010.fits',verbose=False)

dust_T_cold  = hp.read_map(version+'/data/dust_cold_Td_c0002_k00010.fits',verbose=False)
dust_b_cold  = hp.read_map(version+'/data/dust_cold_beta_c0002_k00010.fits',verbose=False)
...
# premise 2019
# tau_prem_353 = hp.read_map('/home/daniel/Data/premise/tau.fits',verbose=False)
# beta_prem    = hp.read_map('/home/daniel/Data/premise/beta.fits',verbose=False)
# Td_prem      = hp.read_map('/home/daniel/Data/premise/temp.fits',verbose=False)


# Meisner/Finkbeiner - 2014
tau_mf_545   = hp.read_map('/home/daniel/Data/meisner-finkbeiner/meisner_finkbeiner_tau_545_0064.fits',verbose=False)
...
def HI_maps():
	...
	fig, (((ax1), (ax2),(ax3)), ((ax4), (ax5), (ax6))) = plt.subplots(3,3)
	...
	plt.sca(ax5)
	hp.mollview(np.log10(map5),cmpa=pl_cmap,hold=True,min=mini,max=maxi, title=r'M&F 2014')
	plt.sca(ax6)
	hp.mollview(np.log10(map6),cmpa=pl_cmap,hold=True,min=mini,max=maxi, title=r'\texttt{premise]')
	# plt.savefig(version+'/nhi/'+file, dpi=1000,bbox_inches='tight')
	plt.show()
	plt.close()
...
def HI_scatter():
	...
	hp.mollview(HI_masked,cmap=pl_cmap,min=0,max=4e20,title='Masked HI')
	plt.savefig('HI_masked',dpi=300,bbox_inches='tight')
	plt.show()
	exit()

	tau_HI_2d  = dummy1[np.where(True != np.isnan(HI_newz))]
	...
	HI_new_new = HI_newz[np.where(True != np.isnan(HI_newz))]

	fig, ((ax1, ax2, ax3, ax4), (ax5, ax6, ax7, ax8)) = plt.subplots(2,4, sharex=True, sharey='row')
	...
	# -----------------------------------------------------------------------------------------
	H1, xedges1, yedges1 = np.histogram2d(HI_new_new,tau_HI_2d, bins=(xedges1,yedges1),normed=True)
	H1     = H1.T
	X1, Y1 = np.meshgrid(xedges1,yedges1)

	slope1, intercept1, r_value1, p_value1, std_err1 = stats.linregress(HI_new_new,tau_HI_2d)

	mean1,edges, nums = stats.binned_statistic(HI_new_new,tau_HI_2d,bins=xedges1,statistic='mean')
	std1, edges, nums = stats.binned_statistic(HI_new_new,tau_HI_2d,bins=xedges1,statistic='std')
	# -----------------------------------------------------------------------------------------
	# Planck 2015
	# -----------------------------------------------------------------------------------------
	...
	# Meisner/Finkbeiner 2014
	# -----------------------------------------------------------------------------------------
	# H5, xedges1, yedges1 = np.histogram2d(HI_new,tau_HI_mf, bins=(xedges1,yedges1),normed=True)
	# H5     = H5.T

	# slope5, intercept5, r_value5, p_value5, std_err5 = stats.linregress(HI_new,tau_HI_mf)

	# mean5,edges, nums = stats.binned_statistic(HI_new,tau_HI_mf,bins=xedges1,statistic='mean')
	# std5, edges, nums = stats.binned_statistic(HI_new,tau_HI_mf,bins=xedges1,statistic='std')
	# -----------------------------------------------------------------------------------------

	centers = (edges[:-1] + edges[1:])/2

	fig.text(0.57, 0.02, r'NH ${\rm [cm^{-2}]}$', ha='center', size=12)
	...
	ax1.set_title(r'Two-Component MBB',size=10)
	...
	# ax5.set_title(r'Meisner/Finkbeiner',size=10)

	ax5.plot(centers,std1/mean1,'k-')
	ax5.set_ylabel(r'$\sigma/\mu$', size=15)
	ax5.tick_params(axis='both',labelsize=12)
	ax5.set_xlim([0.5e20,3.75e20])
	ax5.set_ylim([0,2])

	ax6.plot(centers,std2/mean2,'k-')

	ax7.plot(centers,std3/mean3,'k-')

	ax8.plot(centers,std4/mean4,'k-')
	
	# ax10.plot(centers,std5/mean5,'k-')

	plt.savefig(version+'/nhi/'+file, dpi=300,bbox_inches='tight')
	# plt.show()
	plt.close()
...
def ebv_maps():
	...
	hp.mollview(np.log10(map2),cmap=pl_cmap,hold=True,min=mini,max=maxi, title=r'Planck 2015 MBB')
	plt.sca(ax3)
	hp.mollview(np.log10(map3),cmap=pl_cmap,hold=True,min=mini,max=maxi, title=r'GNILC MBB')
	plt.sca(ax4)
	hp.mollview(np.log10(map4),cmap=pl_cmap,hold=True,min=mini,max=maxi, title=r'$B_{\nu}(19.6K)$')
	plt.savefig(version+'/a_v/'+file, dpi=300,bbox_inches='tight')
	# plt.show()
	plt.close()
...
def ebv_scatter():
	...
	ax7.plot(centers,std3/mean3,'k-')
	ax8.plot(centers,std4/mean4,'k-')
	plt.savefig(version+'/a_v/'+file, dpi=1000,bbox_inches='tight')
	# plt.show()
	plt.close()
...
# tau_pl = dummy1[np.where(True != np.isnan(HI_masked))]
# tau_mf = dummy2[np.where(True != np.isnan(HI_masked))]
# HI_new = HI_masked[np.where(True != np.isnan(HI_masked))]

# xedges1 = np.arange(0,4,0.04)
# xedges1 = xedges1*1e20
# yedges1 = np.arange(0,2.5,0.025)
# yedges1 = yedges1*1e-5
# ymin    = 0
# ylim    = 2e-5


# fig, ((ax1,ax2),(ax3,ax4)) = plt.subplots(2,2,sharey='row',sharex=True)

# fig.suptitle(r'$\tau_{545}$ Comparison', size=20)


# H1, xedges1, yedges1 = np.histogram2d(HI_new,tau_pl, bins=(xedges1,yedges1),normed=True)
# H1     = H1.T
# X1, Y1 = np.meshgrid(xedges1,yedges1)

# slope1, intercept1, r_value1, p_value1, std_err1 = stats.linregress(HI_new,tau_pl)

# mean1,edges, nums = stats.binned_statistic(HI_new,tau_pl,bins=xedges1,statistic='mean')
# std1, edges, nums = stats.binned_statistic(HI_new,tau_pl,bins=xedges1,statistic='std')

# H2, xedges2, yedges2 = np.histogram2d(HI_new,tau_mf, bins=(xedges1,yedges1),normed=True)
# H2     = H2.T
# X2, Y2 = np.meshgrid(xedges1,yedges1)

# slope2, intercept2, r_value2, p_value2, std_err2 = stats.linregress(HI_new,tau_mf)

# mean2,edges, nums = stats.binned_statistic(HI_new,tau_mf,bins=xedges1,statistic='mean')
# std2, edges, nums = stats.binned_statistic(HI_new,tau_mf,bins=xedges1,statistic='std')
...
# ax1.pcolormesh(X1,Y1,H1,cmap=plt.get_cmap('hot_r'))
# ax1.plot(xedges1,slope1*xedges1+intercept1, color='black', linestyle='--')
# ax1.set_title(r'Planck 2015',size=20)
# ax1.set_ylabel(r'$\tau$', size=25)
# ax1.set_ylim([ymin,ylim])

# ax2.pcolormesh(X2,Y2,H2,cmap=plt.get_cmap('hot_r'))
# ax2.plot(xedges2,slope2*xedges2+intercept2, color='black', linestyle='--')
# ax2.set_title(r'meisner-finkbeiner',size=20)

# ax3.plot(centers,std1/mean1,'k-')
# ax3.set_ylabel(r'$\sigma/\mu$', size=15)
# ax3.tick_params(axis='both',labelsize=12)
# ax3.set_xlim([0.5e20,3.75e20])
# ax3.set_ylim([0,2])

# ax4.plot(centers,std2/mean2,'k-')
# plt.savefig('HI_545_pl_mf_compare',dpi=300,bbox_inches='tight')
# plt.show()
